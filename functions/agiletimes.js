...
// This is what is re-played on Alexa device.'use strict';
// --------------- Helpers that build all of the responses -----------------------
function buildSpeechletResponse(title, output, repromptText, shouldEndSession) {
    return {
        outputSpeech: {
...
function addTimesheet(intent, session, callback) {
    ...
    if (projectSlot && typeof(projectSlot.value) !== 'undefined') {
        ...
        speechOutput = categorySlot && typeof(categorySlot.value) !== 'undefined' ? `Got it. Adding new timesheet to ${project} project for aditya, ${category} category.` : `Got it. Adding new timesheet to ${project} project for Aditya`;
    } else {
